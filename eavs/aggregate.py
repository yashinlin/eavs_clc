...
SUM_COLS = [
    "F1a",                               # F1a  — total ballots cast & counted, all modes
    "registered_eligible_voters",         # A1a  — total registered (active + inactive)
    "active_voters",                      # A1b  — EAC-preferred registration denominator
    "total_registrations_received",       # A3a  — registration transaction denominator
    "rejected_registrations",             # A3e  — registration rejection numerator
    "voters_removed_total_2020_2022",     # A9a  — purge numerator
    "mail_transmitted_total",             # C1a
    "mail_returned_by_voters",            # C1b  — mail rejection denominator (2020 proxy for C8a)
    "mail_ballots_rejected_total",        # C9a  — mail rejection numerator
    "provisional_ballots_cast_total",     # E1a  — provisional rejection denominator
    "provisional_ballots_rejected_total", # E1d  — use E1d not E3a (stable across all years)
]
...
def aggregate_state_rates() -> pd.DataFrame:
    ...
    merged["purge_rate"]                 = _safe_rate(merged["voters_removed_total_2020_2022"],  merged["registered_eligible_voters"])
    merged["mail_rejection_rate"]        = _safe_rate(merged["mail_ballots_rejected_total"],     merged["mail_returned_by_voters"])
    merged["provisional_rejection_rate"] = _safe_rate(merged["provisional_ballots_rejected_total"], merged["provisional_ballots_cast_total"])
